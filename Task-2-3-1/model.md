...
### Проверка codestyle (Подзадача при генерации отчета)
    Если пользователь запросит проверку код-стайла, 
    то для задачи вызывается метод checkCodestyle(taskName)
...
* Ключи:
    ...
    - fullReport <GROUPNUMBER> -> Полный отчет с двумя таблицами для группы <GROUPNUMBER>
    - attendanceOnly <GROUPNUMBER> -> Отчет по посещаемости студентами группы <GROUPNUMBER>
    - performanceOnly <GROUPNUMBER> -> Отчет по успеваемости студентами группы <GROUPNUMBER>
    
Порядок ключей не важен

__Примеры:__
```
task-2-3-1 -c "D:\config\config.nsu" -o "D:\reports\report_01.10.2000.html" report 18213
```

```
task-2-3-1 report 18214
```